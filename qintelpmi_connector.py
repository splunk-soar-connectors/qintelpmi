...
class QintelPmiConnector(BaseConnector):
    ...
    def _handle_test_connectivity(self, param):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, ERR_TEST_CONN.format(err=str(e)))
    ...
    def _process_cve_observations(self, data):
        ...
        for obs in observ:
            ...
            if actor_label:
                obs_return["actor"] = reduce_item(actor_label[0], ("attributes", "label"))

            actor_type = reduce_item(obs, ("relationships", "tags", "data"))
            if actor_type:
                obs_return["actor_type"] = reduce_item(actor_type[0], ("attributes", "tag_type"))

            obs_return["exploit_type"] = reduce_item(obs, ("attributes", "exploit_type"))
            ...
            if timestamps:
                for t in timestamps:
                    if t["context"] == "observed":
                        obs_return["observation_date"] = make_timestamp(t["value"]).strftime(DATE_FORMAT)
    ...
    def _process_cve_attributes(self, data, cve):
        ...
        ret_data["affected_system"] = reduce_item(data, ("attributes", "affected_system", "name"))

        ret_data["affected_versions"] = reduce_item(data, ("attributes", "affected_system", "versions"))

        if isinstance(ret_data["affected_versions"], list):
            ret_data["affected_versions"] = ", ".join(ret_data["affected_versions"])

        ret_data["last_observed"] = make_timestamp(reduce_item(data, ("attributes", "last_observed"))).strftime(DATE_FORMAT)
    ...
    def _handle_pmi_get_cve_intel(self, param):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, ERR_PROCESS_RV.format(err=str(e)))

        action_result.set_summary({"observation_count": len(obvs_data), "last_observed": ret_data.get("last_observed")})
